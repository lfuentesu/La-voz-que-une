...
# ==========================================
# FUNCIONES AUXILIARES DE IMAGEN Y DATOS
# ==========================================
def mostrar_imagenes_seguras(img_val):
    """Muestra una o múltiples imágenes separadas por coma si existen o son URLs válidas."""
    if pd.isna(img_val) or not str(img_val).strip():
        return
    
    # Separar nombres por comas
    lista_imagenes = [img.strip() for img in str(img_val).split(",") if img.strip()]
    
    for img_path in lista_imagenes:
        # Verificar si es una URL de internet o un archivo local que existe
        if img_path.startswith("http://") or img_path.startswith("https://"):
            st.image(img_path, use_container_width=True)
        elif os.path.exists(img_path):
            st.image(img_path, use_container_width=True)
        else:
            st.warning(f"⚠️ La imagen '{img_path}' no fue encontrada en el repositorio.")
...
with tab_inicio:
    ...
    if df_noticias.empty:
        ...
        for idx, row in df_noticias.iloc[::-1].iterrows():
            ...
            # Despliegue seguro de imágenes
            mostrar_imagenes_seguras(img_val)
...
with tab_editorial:
    ...
    if df_filtered.empty:
        ...
        for idx, row in df_filtered.iloc[::-1].iterrows():
            ...
            mostrar_imagenes_seguras(row.get("Imagen_URL", ""))
...
with tab_admin:
    ...
    if password == CLAVE_ADMIN:
        ...
        # CINTA DESLIZANTE
        st.markdown("---")
        st.subheader("📢 Actualizar Cinta de Noticias Desplazables")
        ...
        with st.form("form_nueva_noticia", clear_on_submit=True):
            ...
            imagen_url = st.text_input("Ruta o enlace de la imagen (Separe varias imágenes con comas, ej: foto1.jpg, foto2.jpg)")
